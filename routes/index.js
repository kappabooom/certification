...
var getNewAddress = require('./create');
var getVerifyKey = require('./register');
require('../lib/db');
...
router.post('/verify', function(req, res, next) {
	...
	Signupdata.find({'Verifykey': req.body.key}, ['TransactionKey'],function (err, docs) {
		  if (err) {
			  return console.error(err); //顯示異常
		  }
		  else{
			  console.log(docs);
			  var TransactionKey = docs[0].toObject().TransactionKey ;
			  console.log(TransactionKey);
			  var keystore = getNewAddress(TransactionKey);
...
router.get('/new', function (req, res) {
	...
	*/ 
	...
	if(!web3.isConnected()){
		...
		var accountsL = web3.eth.accounts;
		console.log(accountsL);
...
//	    }
	...
	*/ 
	...
	setTimeout(function () {
	    ...
	    //去找new.ejs
	    res.render('new', {newAccount: accountsL });
	    
	    //console.log(contractAddress);
		
		// simpleStorage 的 ABI
		//var producecontract = web3.eth.contract(abi).at(contractAddress)
